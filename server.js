...
app.post('/api/pesan', async (req, res) => {
    // Tambahkan opsi_pemotongan di sini
    const { user_id, hewan_id, jumlah, total_harga, jadwal, metode_bayar, opsi_pemotongan } = req.body;
    ...
    try {
        ...
        // Pastikan opsi_pemotongan memiliki nilai default jika kosong
        const jenis_layanan = opsi_pemotongan || 'potong';

        // Update Query Insert
        const query = `
            INSERT INTO pemesanan 
            (user_id, hewan_id, jumlah, total_harga, tanggal_pemesanan, jadwal_pemotongan, metode_bayar, status_pesanan, status_pembayaran, opsi_pemotongan) 
            VALUES (?, ?, ?, ?, NOW(), ?, ?, ?, ?, ?)
        `;
        ...
        const [result] = await db.promise().query(query, [
            ...
            status_pesanan, 
            status_pembayaran,
            jenis_layanan // Masukkan ke database
        ]);

        await db.promise().query("UPDATE hewan SET stok = stok - ? WHERE id = ?", [jumlah, hewan_id]);

        res.status(201).json({ message: "Pesanan berhasil dibuat", id_pesanan: result.insertId });
