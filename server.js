...
// ==========================================
// API: UPLOAD BUKTI BAYAR CICILAN BULANAN
// ==========================================
app.post('/api/pesanan/:id/cicilan/:angsuranKe', uploadBukti.single('bukti'), async (req, res) => {
    ...
    try {
        ...
        if (cek.length === 0) {
            return res.status(404).json({ message: "Data tagihan tidak ditemukan" });
        }
        ...
        // 3. Catat di History Pembayaran (Opsional, agar muncul di riwayat)
        await db.promise().query(
        ...
        // 4. Buat Notifikasi ke Admin
        await createNotifikasi(null, 'admin', 'Pembayaran Cicilan Baru', `User membayar cicilan ke-${angsuranKe} untuk Pesanan #${id}`, 'pembayaran');
...
app.post('/api/pesanan/:id/cicilan/:angsuranKe', uploadBukti.single('bukti'), async (req, res) => {
    ...
    try {
        // 1. Cek apakah tagihan valid di database
        const [cek] = await db.promise().query(
            "SELECT * FROM cicilan WHERE id_pemesanan = ? AND angsuran_ke = ?", 
            [id, angsuranKe]
        );
        ...
        if (cek.length === 0) {
            // Hapus file yang terlanjur diupload jika data tidak ketemu
            if (req.file) fs.unlinkSync(req.file.path);
            return res.status(404).json({ message: "Data tagihan cicilan tidak ditemukan" });
        }

        // 2. Update status cicilan menjadi menunggu verifikasi
        // status diubah jadi 'menunggu_verifikasi' agar admin bisa cek
        const queryUpdate = `
            UPDATE cicilan 
            SET bukti_bayar = ?, status = 'menunggu_verifikasi', tanggal_bayar = NOW() 
            WHERE id_pemesanan = ? AND angsuran_ke = ?
        ...
        // 3. Catat di tabel history_pembayaran (Untuk Riwayat)
        // Kita catat sebagai jenis 'cicilan'
        await db.promise().query(
            "INSERT INTO history_pembayaran (pesanan_id, jenis, nominal, metode, bukti, keterangan) VALUES (?, 'cicilan', ?, 'transfer', ?, ?)",
            [id, cek[0].nominal, bukti, `Pembayaran Angsuran ke-${angsuranKe}`]
        );
        ...
        await createNotifikasi(
            null, // user_id null karena untuk admin
            'admin', 
